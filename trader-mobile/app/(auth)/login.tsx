      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        ...
        >
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Sign in to manage leads, quotes and reputation.</Text>
          </View>

          <View style={styles.form}>
            <Button
              title="Continue with Google"
              onPress={() => Alert.alert('Google sign-in', 'Prototype UI only (not wired yet).')}
              variant="secondary"
              icon={<Globe size={18} color={Colors.text} />}
            />
            <Button
              title="Continue with Apple"
              onPress={() => Alert.alert('Apple sign-in', 'Prototype UI only (not wired yet).')}
              variant="secondary"
